    ...
    public class MDController : Controller
    {
...

        {
            ...
            foreach (var item in data)
            {
                ...
                CH_p_l += CHCalculation(item.EmpId);

                item.CHprofit_Loss = CHCalculation(item.EmpId);
            }
            ...
            if (user.ID == employeeDetails.EmpId)
            {
                ...
                MDtenure = (DateTime.Parse(DateTime.Now.ToString()) - jd1).Days / 30;
                MDprofit = MDtenure * employeeDetails.Salary.Value;
                MD_p_l = CH_p_l - MDprofit;
            }
            //foreach (var item in data)
            //{

            //    item.CHprofit = CHprofit;
            //}
            ViewBag.MDprofit = MD_p_l;
        ...
        public decimal CHCalculation(int id)
        {
        ...
        public ActionResult CHDetails(int Id)
        {
            var data = db.EmployeeDetails.Where(x => x.ReportingManagerId == Id).ToList();
            foreach (var item in data)
            {
